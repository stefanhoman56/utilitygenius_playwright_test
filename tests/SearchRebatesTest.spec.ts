...
test.describe(`Verify Login Functionality`, () => {
  ...
  test(`Verify search rebates page`, async ({ page }) => {
    ...
    await RebatesPage.clickOnSearchButtonAndSearchUtility(
      searchRebatesPageData.utility
    );

    // Step 6: Verify utility page
    const headerTextOfUtilityPage =
      await RebatesPage.getHeaderTextOfUtilityPage();
    await expect(headerTextOfUtilityPage).toContain(
      searchRebatesPageData.utility
    );
    await page.goBack();
    ...
    const headerTextOfCountry =
      await RebatesPage.getHeaderTextOfCountryHeading();
    ...
    const headerTextOfAllUtilityPage =
      await RebatesPage.getHeaderTextOfAllUtilityPage();
