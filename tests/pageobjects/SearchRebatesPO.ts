...
export class SearchRebatesPO {
  ...
  constructor(page: Page) {
    this.page = page;
    this.searchRebatesLink = page.locator("text='Search Rebates' >> nth=0");
    ...
    this.searchUtility = page.locator(
      "//span[text()='All Results']/parent::div//div"
    );
   ...
   */
  async getHeaderText() {
    return await (await this.page.locator("h1 >> nth=0").innerText()).trim();
  }
  /**
   * Click on the select state drop down #
   */
  async clickOnSelectStateDropDownAndType() {
    await this.selectStateDropDown.click({ force: true });
  ...
  async verifyUserNavigatedToTheSelectedSTateLocation() {
    await expect(this.page.url()).toEqual(
      "https://app.utilitygenius.com/location/california"
    );
  }
  ...
  async getHeaderTextOfCountryHeading() {
    return await this.page.$eval("#react-select-2-group-0-heading", (el) =>
      el.textContent.trim()
    );
  }
  /**
  ...
  async getHeaderTextOfAllUtilityPage() {
    return await this.page.$eval("h1 >> nth=0", (el) => el.textContent.trim());
  }
}
