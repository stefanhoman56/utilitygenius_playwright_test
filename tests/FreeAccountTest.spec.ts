...
test.describe("Verifying user can create a free account", () => {
  test(`Signing up`, async ({ page }) => {
    ...
    await expect(CreateFreeAccount.createFreeAccountBtn).toHaveText(
      CreateFreeAccountData.createFreeAccount
    );
    await CreateFreeAccount.createFreeAccountBtn.click();

    //Step3 : Verify google sign up option and Sign up
    await expect(CreateFreeAccount.googleSignupBtn).toHaveText(
      CreateFreeAccountData.googleSignUpButtonText
    );
    await CreateFreeAccount.signUp();
  });

  test(`Email confirmation and account creation confirmation`, async ({
    page,
    context,
  }) => {
